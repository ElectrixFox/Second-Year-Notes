...
\begin{problem}
    Show that the group $F ^ {\times}$
    (where $F$ is a finite field)
    is cyclic.
    
    \begin{solution}
        \begin{enumerate}[label = (\alph*)]
            \item Denote $n = |F ^ {\times}|$
            \[
            n = p_1 ^ {e_1}p_2 ^ {e_2} \dotsi p_k ^ {e_k}
            \]
            $p = p_q, q = p_1 ^ {e_1}$,
            $p$ is any prime dividing $n$,
            $q$ is the maximum power of $p$ in prime decomposition of $n$.
            Define map $F ^ {\times} \to F ^ {\times}$,
            $x \mapsto x ^ {n / q}$.
            Show that this map is a homomorphism of groups.
            More generally:
            $\varphi : x \mapsto x ^ m$ is always homomorphism,
            $\forall m \in \Z$,
            $\varphi(x) = x ^ m$,
            $\varphi(xy) = (xy) ^ m = x ^ my ^ m = \varphi(x)\varphi(y)$.
    
            \item $|\Im{\varphi}| \geq q$.
            Look at $\ker{\varphi} = \{x \in F ^ {\times} \mid x ^ {n / q} = 1\}$
            $|\ker{\varphi}| = \text{number of solutions to $x ^ {n / q} = 1$}$ same as roots of polynomial $x ^ {n / q} - 1 = 0$ in $F$.
            Degree is equal to $n / q \implies$ number of solutions $\leq \frac{n}{q}$ so $|\ker{\varphi}| \leq \frac{n}{q}$.
            FIT:
            $F ^ {\times} / \ker{\varphi} \cong \Im{\varphi}$.
            In particular,
            \[
            |\Im{\varphi}| = \frac{|F ^ {\times}|}{|\ker{\varphi}|} \geq \frac{n}{n / q} = q.
            \]

            \item There is an element of $F ^ {\times}$ of order $q$.
            $y \in \Im{\varphi}$:
            $y = x ^ {n / q}$ for some $x \in F ^ {\times}$.
            \[
            y ^ q = (x ^ {n / q}) ^ q = x ^ n = x ^ {|F ^ {\times}|} = 1
            \]
            (as the order of any element of a group divides the size of the group).
            Count how many elements of $\Im{\varphi}$ can have order $< q$.

            Recall $q = p ^ {\ell}$,
            $p$ prime.
            $y \in \Im{\varphi}$,
            $y ^ q = 1$ so $\ord(y) \mid q$.
            So $\ord(y) = p ^ i$ for some $i = 0, 1, \dotsc, p - 1$.
            
            If $\ord(y) = p ^ i$,
            then $y ^ {(p ^ i)} = 1$ so $y$ is a root,
            $y ^ {(p ^ i)} - 1 = 0$ thus at most $p ^ i$ solutions.
            Thus,
            the number of elements of $\Im{\varphi}$ of order $< q$ does not exceed
            \[
            p ^ 0 + p ^ 1 + p ^ 2 + p ^ 3 + \dotsi + p ^ {\ell - 1} = \frac{p ^ {\ell} - 1}{p - 1} = \frac{q - 1}{p - 1} < q.
            \]
            So the number of elements of $\Im{\varphi}$ of order $< q$ is $< q$.
            \[
            |\Im{\varphi}| \geq q
            \]
            by $(b)$.
            Therefore,
            $\exists y \in \Im{\varphi}$ such that $\ord(y) = q$.

            Remark:
            if $y ^ {p ^ i} = 1$ then
            \[
            y ^ {p ^ {i + 1}} = y ^ {p ^ i \cdot p} = (y ^ {p ^ i}) ^ p = 1.
            \]
            So,
            it is sufficient to show that the number of elements $\{y \mid y ^ {p ^ {n - 1}} = 1\} < q$.
            
            $|F ^ {\times}| = n$,
            $n = p_1 ^ {\ell_1} \dotsi p_k ^ {\ell_k} = q_1\dotsi q_k$.
            For any $q_i$ there exists $y_i \in F ^ {\times}$ such that $\ord(y_i) = q_i$.

            \item Show $F ^ {\times}$ is cyclic.
            \[
            \ord(y_1y_2) = q_1q_2
            \]
            (as $\gcd(q_1, q_2) = 1$)
            \[
            \ord(y_1y_2y_3) = q_1q_2q_3
            \]
            (as $\gcd(q_1, q_2, q_3) = 1$)
            \[
            \ord(y_1y_2 \dotsi y_{\ell}) = q_1q_2\dotsi q_{\ell} = n = |F ^ {\times}|
            \]
            so $F ^ {\times} = \langle y_1, y_2, \dotsc, y_{\ell}\rangle$ so cyclic.
        \end{enumerate}
    \end{solution}
\end{problem}
...
\begin{problem}
    $\mathrm{SL}_2(F)$,
    $F$ a field.
    Then $\mathrm{SL}_2(F)$ is generated by elements of the form
    \[
    \begin{pmatrix}
        0 & u \\ 0 & 1
    \end{pmatrix},
    ...
    \begin{solution}
        ...
        then $\begin{pmatrix}
            1 & 0 \\ 1 & 1
        \end{pmatrix}A$ has $c' \neq 0$ which goes to the previous case.
        \[
        \begin{pmatrix}
            a & b \\ c & d
        \end{pmatrix}
        \]
        $A_1A_2A_3\dotsc A_kA = I$.
        $A = A_k ^ {-1}A_{k - 1}^{-1}\dotsi A_2 ^ {-1}A_1 ^ {-1}$.
        But $\begin{pmatrix}
            1 & u \\ 0 & 1
